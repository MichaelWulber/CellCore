...
class NPolygon {
  ...
  public NPolygon[] split() {
    ...
    for (int i = 0; i < vertices.length - 1; ++i) {
      midpoints[i] = weightedAverage(vertices[i], vertices[i + 1], random(0.3, 0.7));
    }
    midpoints[vertices.length - 1] = weightedAverage(vertices[vertices.length - 1], vertices[0], random(0.3, 0.7));
    ...
    indexA = getLongestEdge() * 2 + 1;
    indexB = (indexA + pool.length/2) % pool.length;
  ...
  // returns index of starting vertex of the longest edge
  private int getLongestEdge() {
    float max = 0;
    float dval = 0;
    int index = 0;
    for (int i = 0; i < vertices.length - 1; ++i) {
      dval = dist(vertices[i].x, vertices[i].y, vertices[i + 1].x, vertices[i + 1].y);
      if (dval > max) {
        max = dval;
        index = i;
      }
    }
    dval = dist(vertices[vertices.length - 1].x, vertices[vertices.length - 1].y, vertices[vertices.length - 1].x, vertices[vertices.length - 1].y);
    if (dval > max) {
