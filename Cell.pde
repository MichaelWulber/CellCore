...
class Cell {
  ...
  Cell(NPolygon poly, color col) {
    ...
    this.pallet = new Pallet(color(92, 240, 255), color(151, 250, 159), color(159, 243, 214));
    
    this.level = 0;
    this.limitLevel = (int)random(5, 6);
    
    generateCore();
  }
  
    Cell(NPolygon poly, color insideColor, float thickness, float theta, int level) {
    this.poly = poly;
  ...
  private void renderCore() {
    fill(weightedAverage(insideColor, wallColor, getPercentageFilled()));
    noStroke();
  ...
  public color weightedAverage(color A, color B, float weight) {
    return color(weight * red(A) + (1.0f - weight) * red(B), weight * green(A) + (1.0f - weight) * green(B), weight * blue(A) + (1.0f - weight) * blue(B));
  }
  ...
  public float getPercentageFilled() {
     return calcArea(coreControls)/calcArea(wallControls);
  }
  
  // split cell into two fairly equal size cells
  public Cell[] split() {
    NPolygon[] polygons = poly.split();
    return new Cell[] {new Cell(polygons[0], insideColor, wallThickness * 0.8, min(theta[0] + random(-0.01, 0.0), 0.15), level++), new Cell(polygons[1], insideColor, wallThickness * 0.8, min(theta[0] + random(-0.01, 0.0), 0.15), level++)};
  }
}